...
def makeInfoResponse(slot):
    ...
    crc = calculate_crc32(output)
    ustruct.pack_into('<L', output, 8, crc)
    return output

packetNumber = 0
microseconds = 0
before = utime.ticks_us()

def makeDataResponse(ax, ay, az, pitch, yaw, roll):
    global packetNumber, microseconds, before
    ...
    ustruct.pack_into('<L', output, 32, packetNumber)
    packetNumber += 1
    ...
    # micro time passed
    now = utime.ticks_us()
    microseconds += (now - before) // 1000
    before = now
    # writeUInt48LE
    ustruct.pack_into('<Q', output, 68, microseconds & 0xFFFFFFFFFFFF)
    # -- gyro shake
    # Acceleration X
